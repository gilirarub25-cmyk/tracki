...
// Helper: color según el valor y el tipo
const colorPorValor = (
  valor: number,
...
const TxSummaryGroup = ({ ingresos, gastos, ahorro, tasa }: { ingresos: number, gastos: number, ahorro: number, tasa: number }) => (
  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
    <div className={`${glassCard} p-6`}>
      ...
      <h2 className={`text-3xl font-bold ${colorPorValor(ingresos, "ingreso")}`}>
        {formatoEUR(ingresos)}
      </h2>
    ...
    <div className={`${glassCard} p-6`}>
      ...
      <h2 className={`text-3xl font-bold ${colorPorValor(gastos, "gasto")}`}>
        {formatoEUR(gastos)}
      </h2>
      <p className="text-xs text-[#bbcabf] mt-2">Basado en tus registros</p>
    </div>
    <div className={`${glassCard} p-6 relative overflow-hidden`}>
      ...
      <h2 className={`text-3xl font-bold relative z-10 ${colorPorValor(ahorro, "balance")}`}>
        {formatoEUR(ahorro)}
      </h2>
...
const TxRow = ({ tx }: { tx: any }) => {
  ...
  const iconName = tx.categoria_icono || (isPositive ? 'income' : 'expense');
  const iconColor = isPositive ? 'text-[#4edea3]' : 'text-[#ffb4ab]';
  ...
  return (
    <div className="grid grid-cols-12 gap-4 py-4 border-b border-[#3c4a42]/40 items-center hover:bg-[#1a211d]/50 transition-colors px-4 rounded-lg -mx-4">
      <div className="col-span-6 md:col-span-5 flex items-center gap-4">
        <div className="w-10 h-10 rounded-xl bg-[#1a211d] flex items-center justify-center border border-[#3c4a42]">
      ...
      <div className={`col-span-6 sm:col-span-3 md:col-span-2 text-right font-bold ${isPositive ? 'text-[#4edea3]' : 'text-[#ffb4ab]'}`}>
        {isPositive ? '+' : '-'}{formatoEUR(tx.cantidad)}
      </div>
    </div>
  );
};
