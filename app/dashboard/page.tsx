...
export default function DashboardIndex() {
  ...
  const [balance, setBalance] = useState(0); // Balance real: balance_inicial + Σingresos − Σgastos
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // Balance real: suma de balance_inicial de las cuentas filtradas
      // más todas las transacciones históricas (no solo las del mes).
      ...
      let queryAll = supabase.from("transacciones").select("monto, tipo");
      if (selectedAccountId !== null) queryAll = queryAll.eq("id_cuenta", selectedAccountId);
      const { data: txAll } = await queryAll;
      (txAll || []).forEach((t: any) => {
